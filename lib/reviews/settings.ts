...
/**
 * Everything Osmin can change from /dashboard/settings. Pure — no database —
 ...
 * "use the default here", so defaults live in code only and the owner guide
 * PDF, a fresh database and a missing row all behave identically.
 */
export interface ReviewSettings extends Cadence {
  ...
  /** Where replies to review emails go. Null = REVIEW_REPLY_TO, else CONTACT_EMAIL. */
  replyTo: string | null;
  /** Who gets unhappy-customer alerts from /feedback. Empty = CONTACT_EMAIL. */
  alertEmails: string[];
}
