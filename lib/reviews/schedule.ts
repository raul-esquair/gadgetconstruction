...
/**
 * How many emails a customer gets and how far apart. Osmin edits this from
 ...
 * `completedAt` may be null (Osmin doesn't always remember the exact day).
 */
export function resolveStartAt(completedAt: string | null, today: string): string {
  if (!completedAt) return addDays(today, 1);

  /**
   * Finished today — Osmin is marking it complete at the walkthrough, so send
