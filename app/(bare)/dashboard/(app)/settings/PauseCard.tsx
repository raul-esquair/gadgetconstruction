...
export default function PauseCard({
  ...
  return (
    <div
      className={
        paused
          ? "rounded-xl border border-amber-300 bg-amber-50 p-5 sm:p-6"
          : "rounded-xl border border-neutral-200 bg-white p-5 sm:p-6"
      }
    >
      <div className="flex items-start justify-between gap-6">
        <div>
          <label htmlFor="sending-switch" className="font-heading text-lg font-bold text-primary cursor-pointer">
            {paused ? "Review emails are paused" : "Review emails are on"}
          </label>
          <p className="mt-1 text-sm text-neutral-500 max-w-2xl" aria-live="polite">
            {paused ? (
              <>
