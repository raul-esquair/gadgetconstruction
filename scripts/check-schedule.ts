...
// Same-day: Osmin is marking it complete at the walkthrough, so send while
...
console.log("\n— email templates —");
{
  ...
  check("default subject 1 fills {project}", e1.subject, "A favor about your new deck");
  check("default body 1 fills {when_finished}", e1.text.includes("We finished your new deck a few days ago."), true);
  check("same day reads 'today'", renderReviewEmail({ ...jane, completedAt: TODAY }, 1, TODAY).text.includes("your new deck today."), true);
  ...
  check("empty token links to bare /feedback", e1.text.includes("gadgetconstructionsf.com/feedback\n"), true);
...
console.log("\n— settings validation —");
{
  ...
  const badEmail = validateSettings({ ...base, replyTo: "osmin@", alertEmails: ["a@b.com", "nope"] });
